...
def Compute_FDR(psmlist, user_FDR, PSMscore_col, FDR_calc_score_direction, prot_col, decoy_prefix):
    ...
    if len(target_scores)==0:
        print( 'target score length == 0')
        return 'cannot reach specified FDR'     
        
    if len(decoy_scores)==0:    
        decoy_PSMs = []
        target_peptides = [z[1] for z in target_peptides]
        if FDR_calc_score_direction ==1:
            FDR_cutoffscore = min(target_scores)
        elif FDR_calc_score_direction==-1:
            FDR_cutoffscore = max(target_scores)
    ...
    for s in scores:
        ...
        if begin==1:
            ...
            if FDR_cutoffscore=='na' and currentrate<=user_FDR:
                FDR_cutoffscore = s[0]
                
            if FDR_cutoffscore!='na':
                FDR_values.append((s, pass_decoy_scores, pass_target_scores, currentrate))

    if skipfile ==1 or FDR_cutoffscore=='na':
        print( 'skipfile: ', skipfile, 'FDR_cutoffscore: ', FDR_cutoffscore)
        return 'cannot reach specified FDR'
